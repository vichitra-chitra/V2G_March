# app.py  — Streamlit wrapper for S.KOe COOL V2G Optimisation (v5)
# Deploy to: Hugging Face Spaces (SDK: Streamlit)
# Requirements: streamlit, numpy, pandas, matplotlib, scipy
...
st.title("S.KOe COOL — Smart & Bidirectional Charging Optimisation")
st.caption("TU Dortmund IE³ × Schmitz Cargobull AG | Master's Thesis 2026")

st.markdown("""
This tool optimises charging and V2G (Vehicle-to-Grid) discharge schedules
for the Schmitz Cargobull S.KOe COOL electric reefer trailer.
Upload your SMARD electricity price CSV and set the trailer parameters below.
""")
...
# ── Sidebar: Parameters ──────────────────────────────────────────────────────
with st.sidebar:
    st.header("Trailer Parameters")
    st.caption("S.KOe COOL — 70 kWh battery, 22 kW AC bidirectional")

    uploaded_file = st.file_uploader(
        "Upload SMARD price CSV (DE/LU 15-min day-ahead)",
        type=["csv"],
        help="Download from smard.de → Market data → Day-ahead prices → Germany/Luxembourg"
    )

    st.subheader("Weekday schedule")
    arrival_h    = st.slider("Arrival time (hour)",   10, 23, 16, 1,
                              help="Time trailer returns to depot")
    departure_h  = st.slider("Departure time (hour)",  1,  9,  6, 1,
                              help="Time trailer leaves depot next morning")
    soc_init_pct = st.slider("Arrival SoC (%)",       20, 90, 45, 5,
                              help="Battery charge level on arrival")

    st.subheader("Scenarios to run")
    run_smart = st.checkbox("B — Smart charging (no V2G)", value=True)
    run_milp  = st.checkbox("C — MILP Day-Ahead",          value=True)
    run_mpc   = st.checkbox("D — MPC receding horizon",    value=False,
                             help="Adds ~2 min compute per day-type")

    st.subheader("Day types")
    show_winter_wd = st.checkbox("Winter weekday", value=True)
    show_summer_wd = st.checkbox("Summer weekday", value=True)
    show_winter_we = st.checkbox("Winter weekend (48h)", value=False)
    show_summer_we = st.checkbox("Summer weekend (48h)", value=False)

    run_btn = st.button("Run optimisation", type="primary", use_container_width=True)

# ── Main panel ────────────────────────────────────────────────────────────────
if not uploaded_file:
    st.info("Upload your SMARD CSV file in the sidebar to get started.")
    st.markdown("""
    **How to get the CSV:**
    1. Go to [smard.de](https://www.smard.de/en/marktdaten)
    2. Select: Wholesale prices → Day-ahead → Germany/Luxembourg
    3. Download as CSV (quarterly resolution = 15-min)
    """)
    st.stop()
...
with st.spinner("Loading price data..."):
    # Save uploaded file to a temp path for your existing loader
    import tempfile, os
    ...
    try:
        _load_csv_raw(csv_path)   # warm the cache
...
CONFIGS = []
if show_winter_wd: CONFIGS.append(("Winter Weekday", WINTER_M, False, False))
if show_summer_wd: CONFIGS.append(("Summer Weekday", SUMMER_M, False, False))
if show_winter_we: CONFIGS.append(("Winter Weekend", WINTER_M, True,  True))
if show_summer_we: CONFIGS.append(("Summer Weekend", SUMMER_M, True,  True))
...
for (label, months, is_wknd, is_48h) in CONFIGS:
    ...
    with st.spinner(f"Optimising {label}..."):
        try:
            ...
            if is_48h:
                ...
                Pc,Pd,soc = run_A_dumb(v2g, buy_48, v2gp_48, W, E_init)
                A = make_kpi("A - Dumb", v2g, Pc, Pd, soc, buy_48, v2gp_48,
                             E_init, is_weekend_48=True)
                results = [A]
                if run_smart:
                    Pc,Pd,soc = run_B_smart(v2g, buy_48, v2gp_48, E_init)
                    results.append(make_kpi("B - Smart (no V2G)", v2g, Pc, Pd,
                                           soc, buy_48, v2gp_48, E_init,
                                           is_weekend_48=True))
                if run_milp:
                    Pc,Pd,soc = run_C_milp(v2g, buy_48, v2gp_48, E_init)
                    results.append(make_kpi("C - MILP Day-Ahead", v2g, Pc, Pd,
                                           ...
                                           is_weekend_48=True))
                if run_mpc:
                    with st.spinner(f"Running MPC ({W} sub-problems)..."):
                        Pc,Pd,soc = run_D_mpc(v2g, buy_48, v2gp_48, E_init)
    ...
    kpi_data = {
        "Scenario"             : [r["label"] for r in results],
        "Net cost (EUR/day)"   : [f"{r['net_cost']:.4f}"    for r in results],
        "Charge cost (EUR/day)": [f"{r['charge_cost']:.4f}" for r in results],
        "V2G revenue (EUR/day)": [f"{r['v2g_rev']:.4f}"     for r in results],
        "V2G export (kWh/day)" : [f"{r['v2g_kwh']:.2f}"     for r in results],
        "Savings vs Dumb/day"  : [
            "—" if i == 0 else f"EUR {ref - r['net_cost']:+.4f}"
            for i, r in enumerate(results)
        ],
        "Annual savings (x365)": [
            "—" if i == 0 else f"EUR {(ref - r['net_cost'])*365:+,.0f}"
            for i, r in enumerate(results)
        ],
    }
    ...
    buf2.seek(0)
    st.download_button(
        label=f"Download {label} chart (PNG)",
        data=buf2,
        file_name=f"v2g_{label.lower().replace(' ','_')}.png",
        mime="image/png"
    )
    st.divider()

st.caption(
    "S.KOe COOL V2G Optimisation · TU Dortmund IE³ × Schmitz Cargobull AG · 2026 · "
    "Confidential — thesis research only"
)
